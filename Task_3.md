...
// if a table is of count 1000 and granularity is 10
// returns table at index 0,100,200,300...900

// within a larger function, something like:
// where sym(s), start/end times (st;et) and granularity(g) are passed from gateway/callimg service
...
- If we assume 2 million data points per sym per day (on average for the universe of syms - some being traded less heavily than others), then for 5000 syms for one date that's approximately a total of 100ms*5000, or 750 seconds (12.5mins) execution time. That's for one core. 
- Assuming 250 trading days per year, in theory that's ~52 hours needed for the execution time of 1 year of data. Utilise 3 cores, with peaching, reduces execution time down to ~11hours for 1 year of data, or ~52 hours for the full 3 years of data. 
